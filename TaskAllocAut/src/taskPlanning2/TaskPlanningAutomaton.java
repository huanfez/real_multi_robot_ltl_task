...
/** 
 * Synthesize a subtask planning automaton: 
...
public class TaskPlanningAutomaton {
	
	public VtxOfProdAut initState;
	public List<VtxOfProdAut> acceptStates = new ArrayList<>();
	public DirectedWeightedPseudograph<VtxOfProdAut, EdgOfProdAut> taskAllocAutGraph = new DirectedWeightedPseudograph<>(EdgOfProdAut.class);
	
	public TaskPlanningAutomaton() {};
	
	public TaskPlanningAutomaton(VtxOfProdAut initState, List<VtxOfProdAut> acceptStates, 	DirectedWeightedPseudograph<VtxOfProdAut, EdgOfProdAut> taskAllocAutGraph) {
	 ...
	 * **/
	public TaskPlanningAutomaton(RobotTransitionSystem transitionSystem, TaskAutomaton taskAutomatonT, Set<Robot> providedRobots) {
		/* *Claim for task allocation automaton* */
		//TaskAllocAutomaton taskAllocationAut = new TaskAllocAutomaton();
		DirectedWeightedPseudograph<VtxOfProdAut, EdgOfProdAut> productAutomaton = new DirectedWeightedPseudograph<>(EdgOfProdAut.class);
		DirectedWeightedPseudograph<VtxOfProdAut, EdgOfProdAut> taskAllocAutGraphOfPA = new DirectedWeightedPseudograph<>(EdgOfProdAut.class);
		...
		/* 1.1 convert Automaton (class) to TaskAutomaton (class)  */
		...
		/* 1.2 successor set for task automaton's vertices and transition system's vertices */
		...
		/* 1.3 init state of task planning auitomaton*/
		AutomatonEdge initStateEdge;
		String qualifiedInitStateSuccessor = new String();
		for (String initStateSuccessor : neighborsOfAut.successorsOf(taskAutInitStat)){
			
			initStateEdge = taskAutomatonT.AutomatonGraph.getEdge(taskAutInitStat, initStateSuccessor);
			System.out.println("Successor states are: " + transitionSystem.initSate + initStateEdge.Event);
			if(stateLabelVerify(transitionSystem.initSate, initStateEdge)){
				qualifiedInitStateSuccessor  = initStateSuccessor;
				break;
			}
		}
		
		/* 1.3 Synthesize the composite vertex of product automaton (subtask allocation automaton) */
...
//				System.out.println("Current vertex is:" + tmpVertexOfPA.TsState + tmpVertexOfPA.AutState);
				
				//Identify the initial state
				if (tranSystemVertex.equals(transitionSystem.initSate) && automatonVertex.equals(qualifiedInitStateSuccessor))
		...
		for (VtxOfProdAut strtVertex : productAutomaton.vertexSet())
			for (VtxOfProdAut destVertex : productAutomaton.vertexSet()) {
				...
				if (TsTransition && AutTransition) {
					...
					Set<AutomatonEdge> tmpAutEdgeSet = taskAutomatonT.AutomatonGraph.getAllEdges(strtVertex.AutState, destVertex.AutState);
					for (AutomatonEdge autTransEdge : tmpAutEdgeSet)
						if (stateLabelVerify(destVertex.TsState, autTransEdge)) {
							EdgOfProdAut edgeOfProdAut = new EdgOfProdAut();
//							edgeOfProdAut.action = autTransEdge.Event;
							edgeOfProdAut.action = tsEdge.process;
							edgeOfProdAut.weight = 0;
							
							for (int actionIndex = 0; actionIndex<edgeOfProdAut.action.length(); actionIndex++){
								char singleAcation = edgeOfProdAut.action.charAt(actionIndex);
								
								for (int index = 0; index < destVertex.TsState.size(); index++)
									if (destVertex.TsState.get(index).equals(Character.toString(singleAcation))) {
										edgeOfProdAut.robotID.add(transitionSystem.idList.get(index));
										edgeOfProdAut.weight += findWeightOfRobotAction(Character.toString(singleAcation), transitionSystem.idList.get(index), providedRobots);
										destVertex.robotState.set(index, true);
									}
							}
							
							productAutomaton.addEdge(strtVertex, destVertex, edgeOfProdAut);
//							System.out.println(strtVertex.TsState + strtVertex.AutState +"---" + autTransEdge.Event + destVertex.TsState + destVertex.AutState);
							productAutomaton.setEdgeWeight(edgeOfProdAut, edgeOfProdAut.weight);
//							System.out.println("verified weight of  " + edgeOfProdAut.action + " are:" + edgeOfProdAut.weight);
						}
				}
			}
	    ...
	    for (VtxOfProdAut vtx: productAutomaton.vertexSet())
	    	System.out.println("vextex is " + vtx.TsState + vtx.AutState);
...
//	    taskAllocAutGraphOfPA = productAutomaton;
	...
	/** Verify if the state labelling satisfies an event */
	private boolean stateLabelVerify(ArrayList<String> compositeSate, AutomatonEdge AutEdge){
		boolean satisfaction = true;
		
		Set<String> compositeStateStringSet = new HashSet<>(compositeSate);
		compositeStateStringSet.remove("i");
		
		if (compositeStateStringSet.isEmpty())
			satisfaction = false;
		else
			for (String singleState : compositeStateStringSet)
				if (!singleState.equals(AutEdge.Event)){
					satisfaction = false;
					break;
				}
		
//		if (!compositeStateStringSet.contains(AutEdge.Event))
//			satisfaction = false;
	...
	/** Output the subtask allocation automaton graph in .dot format */
    public static void renderProdAutGraph2(Graph<VtxOfProdAut, EdgOfProdAut> PAGraph, String fileName)
            throws ExportException, IOException
    {
		ComponentNameProvider<VtxOfProdAut> vertexLabelProvider = new ComponentNameProvider<VtxOfProdAut>(){
			public String getName(VtxOfProdAut vertex) 
		...
		ComponentNameProvider<VtxOfProdAut> vertexIdProvider = new ComponentNameProvider<VtxOfProdAut>() {
			public String getName(VtxOfProdAut vertex) {
				...
				for (int index =0; index < vertex.AutState.length(); index++)
				{
					 if (vertex.AutState.charAt(index) == ',')
						 listString += '_';
					 else
						 listString += vertex.AutState.charAt(index);
				}
				return listString;
			}
		};
	...
	public GraphPath<VtxOfProdAut, EdgOfProdAut> lowestCostPath () {
		...
		if (!taskAllocAutGraph.vertexSet().isEmpty()) {
			DijkstraShortestPath<VtxOfProdAut, EdgOfProdAut> dijkstraAlg =  new DijkstraShortestPath<>(taskAllocAutGraph);
	...
	/** Output the lowest cost path
	 * @throws IOException */
	public static void outputFinalOptimalPath(List<List<GraphPath<VtxOfProdAut,EdgOfProdAut>>> ListOfShortestPathOfSubAllocationLists,
			String fileNameOfShortestPath) 
			throws IOException {
