...
public class experiment_c1s2 {
	...
	public static void main(String args[]) throws ExportException, IOException {
		...
		final RegExp regExpr_1 = new RegExp("((d((ef)|(fe)))|(e((df)|(fd)))|(f((de)|(ed))))g");
...
//			add(regExpr_4.toAutomaton());
		...
		RobotConfig robotConfig002 = new RobotConfig(decompositionSetConvert.get(0).get(0), new HashSet<Robot>(Arrays.asList(robot2)));
		
		RobotConfig robotConfig011 = new RobotConfig(decompositionSetConvert.get(0).get(1), new HashSet<Robot>(Arrays.asList(robot2)));
		RobotConfig robotConfig012 = new RobotConfig(decompositionSetConvert.get(0).get(1), new HashSet<Robot>(Arrays.asList(robot3)));
		
		RobotConfig robotConfig021 = new RobotConfig(decompositionSetConvert.get(0).get(2), new HashSet<Robot>(Arrays.asList(robot3)));
		
		Set<RobotConfig>setOfRobotConfigs_0  = new HashSet<>(Arrays.asList(robotConfig002));
		Set<RobotConfig>setOfRobotConfigs_1  = new HashSet<>(Arrays.asList(robotConfig011, robotConfig012));
		Set<RobotConfig>setOfRobotConfigs_2  = new HashSet<>(Arrays.asList(robotConfig021));
		...
		for (RobotSetSwitch robotSetConfig : robotSetSwitchList) {
			...
			for (RobotConfig singleConfig : robotSetConfig.autRobotsPair) {
				...
				System.out.println("Robot is:" + singleConfig.robotSet);
				...
				if (subtaskAllocAutomatonT.acceptStates.contains(subtaskAllocAutomatonT.initState))
					System.out.println("path is with an initial state");
				else{
