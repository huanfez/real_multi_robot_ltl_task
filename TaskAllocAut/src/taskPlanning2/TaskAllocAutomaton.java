...
public class TaskAllocAutomaton {
	...
	public TaskAllocAutomaton(VtxOfProdAut initState, List<VtxOfProdAut> acceptStates, 	DirectedWeightedPseudograph<VtxOfProdAut, EdgOfProdAut> taskAllocAutGraph) {
	 ...
	 * **/
	public TaskAllocAutomaton(RobotTransitionSystem transitionSystem, TaskAutomaton taskAutomatonT, Set<Robot> providedRobots) {
		...
		for (VtxOfProdAut strtVertex : productAutomaton.vertexSet())
			for (VtxOfProdAut destVertex : productAutomaton.vertexSet()) {
				...
				if (TsTransition && AutTransition) {
					...
					System.out.println(tsEdge.process);
						...
						if (tsEdge.process.equals(autTransEdge.Event)) {
							...
							for (int index = 0; index < strtVertex.TsState.size(); index++) 
								if (strtVertex.TsState.get(index).equals(autTransEdge.Event)) {
									...
									edgeOfProdAut.weight += findWeightOfRobotAction(edgeOfProdAut.action, transitionSystem.idList.get(index), providedRobots);
...
//	    	System.out.println(finalAcceptState.TsState + "," + finalAcceptState.AutState);
	...
	/** Output the subtask allocation automaton (a different data type) graph in .dot format */
	...
	/** Output the lowest cost path
	 * @throws IOException */
	public static void outputFinalOptimalPath(List<List<GraphPath<VtxOfProdAut,EdgOfProdAut>>> ListOfShortestPathOfSubAllocationLists, String fileNameOfShortestPath) 
			throws IOException {
