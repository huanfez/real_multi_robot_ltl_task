...
public class RobotTransitionSystem {
	...
	public static RobotTransitionSystem transitionSystemComposition(Set<Robot> robotSubSets, ArrayList<String> CoopEventSet) {
...
//		System.out.println("robot is:" + robot0.tsGraph.vertexSet() + "," + robotSubList.get(1).tsGraph.vertexSet());
		RobotTransitionSystem composedTranSystem = new RobotTransitionSystem(Arrays.asList(robot0.getID()), robot0.initSate, robot0.tsGraph);
	...
	public static Graph<ArrayList<String>, TSEdge> ParallelComposTS(Graph<ArrayList<String>, TSEdge> 
		...
		for (final ArrayList<String> vertexOfTS1 : TS1.vertexSet())
			for (final ArrayList<String> vertexOfTS2 : TS2.vertexSet()) {
		...
		for (ArrayList<String> strtVertex : parallelTS.vertexSet()) {
			...
			ArrayList<String> strtVertexOfTS2 = new ArrayList<String>(strtVertex.subList(strtVertex.size() - 1, strtVertex.size()));
			...
			for (ArrayList<String> destVertex : parallelTS.vertexSet()) {
				...
				ArrayList<String> destVertexOfTS2 = new ArrayList<String>(destVertex.subList(destVertex.size() - 1, destVertex.size()));
...
//					if (CooperativeEventSet.contains(ts1Edge.process))
				if (ts1Edge != null && ts2Edge != null) {
					parallelTS.addEdge(strtVertex, destVertex, new TSEdge(ts1Edge.process+ts2Edge.process));
				}
				else if (ts1Edge != null && ts2Edge == null && strtVertexOfTS2.equals(destVertexOfTS2)) {
					if (!CooperativeEventSet.contains(ts1Edge.process))
						parallelTS.addEdge(strtVertex, destVertex, new TSEdge(ts1Edge.process));
				}
				else if (ts2Edge != null && ts1Edge == null && strtVertexOfTS1.equals(destVertexOfTS1)) {
    ...
    {
		ComponentNameProvider<ArrayList<String>> vertexLabelProvider = new ComponentNameProvider<ArrayList<String>>(){
		...
		ComponentNameProvider<ArrayList<String>> vertexIdProvider = new ComponentNameProvider<ArrayList<String>>() {
            public String getName(ArrayList<String> vertex)
            {
            	String listString = "";
